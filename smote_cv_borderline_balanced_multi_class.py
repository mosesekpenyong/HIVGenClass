...
# Drop the target column named 'Cod_var'
X = df.drop('Cod_var', axis=1)
y = df['Cod_var']
...
borderline_smote = BorderlineSMOTE(sampling_strategy={cls: max_class_count for cls in y.unique()}, random_state=42, kind='borderline-1')

# Initialize SMOTE for both training and test data
smote = SMOTE(sampling_strategy={cls: max_class_count for cls in y.unique()}, random_state=42)

# Initialize an empty DataFrame to store results
results_df = pd.DataFrame(columns=list(X.columns) + ['Cod_var', 'DatasetType'])
...
for fold_index, (train_index, test_index) in enumerate(cv.split(X, y), 1):
    ...
    X_train_resampled['Cod_var'] = y_train_resampled
    ...
    X_test_resampled['Cod_var'] = y_test_resampled
